...
- (void)updateMask{
    ...
    switch (self.defaultMaskType){
        ...
        case  SVProgressHUDMaskTypeOpaque:{
            self.backgroundLayer = [CALayer layer];
            self.backgroundLayer.frame = self.bounds;
            //dukai
            self.backgroundLayer.backgroundColor = [UIColor colorWithWhite:1 alpha:1].CGColor;
            [self.backgroundLayer setNeedsDisplay];
...
- (UIControl*)overlayView{
    if(!_overlayView){
        ...
        _overlayView = [[UIControl alloc] initWithFrame:CGRectMake(0, 65, windowBounds.size.width, windowBounds.size.height)];
#else
