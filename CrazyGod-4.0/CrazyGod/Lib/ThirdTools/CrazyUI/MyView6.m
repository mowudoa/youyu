...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _strokeColor=[UIColor whiteColor];//背景圆环的颜色
        ...
        UIBezierPath *path=[UIBezierPath bezierPathWithArcCenter:CGPointMake(frame.size.width/2, frame.size.height/2) radius:frame.size.width/2-2 startAngle:degreesToRadians(0) endAngle:degreesToRadians(360) clockwise:YES];
        ...
        [gradientLayer1 setColors:[NSArray arrayWithObjects:(id)UIColorFromRGB(0xFD497F).CGColor,(id)[UIColorFromRGB(0x4997FD) CGColor], nil]];
        
        //注意，要把这行注释掉，不然进度条位置偏移 显示不出来
        
        [gradientLayer1 setLocations:@[@0.1,@0.9]];
        [gradientLayer1 setStartPoint:CGPointMake(0.05, 1)];//调整颜色比例主要是调这两个参数（0－1之间）
        [gradientLayer1 setEndPoint:CGPointMake(0.9, 0)];//调整颜色比例主要是调这两个参数（0－1之间）
